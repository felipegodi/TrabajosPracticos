...
foreach file of local manyfiles {
	import delimited "$INPUT\Maryland\Weather/`file'", clear
	* Me aparece también la hora. Lo tengo que separar primero
	*split date, p(" ")
	*drop date_time date_time2
	*ren date_time1 date_time
	* A partir de acá tengo igual que Amely excepto que puedo tener más de una obs x día
	* En 120-122 hago que se quede con una observación de cada día
	*bys date_time: gen n=_n 
	*keep if n==1
	*drop n
	split date, p(-)
